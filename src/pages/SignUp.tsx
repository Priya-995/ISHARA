    ...
    const renderInitialStep = () => (
        <div className="w-full max-w-sm">
             <header className="mb-10 text-center">
                <img src="/lk5GFyFTTvqYupHbQQDAQQ-removebg-preview.png" alt="Ishara Logo" className="w-24 h-24 mx-auto" />
            </header>

            <main className='px-4'>
                <h1 className="text-3xl font-extrabold mb-8 text-center">Create an account</h1>
                <div className="space-y-3 flex flex-col items-center">
                    <Button onClick={handleGoogleSignIn} className="w-full max-w-xs bg-white text-black dark:text-gray-900 font-semibold rounded-full h-11 justify-center items-center flex hover:bg-gray-200">
                        <GoogleIcon />Sign up with Google
                    </Button>
                    {/* Removed Apple sign up button */}
                    <div className="flex items-center py-2 w-full max-w-xs mx-auto">
        ...
        <div className="w-full max-w-sm">
            ...
            <div className="w-full text-left px-4">
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                    ...
                    <div className="space-y-2">
                        <Input id="password" type={showPassword ? "text" : "password"} placeholder="Password" required value={password} onChange={e => setPassword(e.target.value)} className="bg-transparent border-gray-700 rounded-md h-14 px-3 text-lg" />
                    </div>
                    <div className="space-y-2">
                        <Input id="confirmPassword" type={showConfirmPassword ? "text" : "password"} placeholder="Confirm Password" required value={confirmPassword} onChange={e => setConfirmPassword(e.target.value)} className="bg-transparent border-gray-700 rounded-md h-14 px-3 text-lg" />
                    </div>
